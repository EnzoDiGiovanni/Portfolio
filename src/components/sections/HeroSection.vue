...
// --- TypeWriter ---
const typewriterWords = ["Développeur Full-Stack", "Problem Solver"];
  ...
  if (!isDeleting.value) {
    ...
    if (currentText.value.length > 0) {
      ...
      currentWordIndex.value =
        (currentWordIndex.value + 1) % typewriterWords.length;
      typewriterTimer = setTimeout(typewriterTick, 100);
    }
  }
}
...
<template>
  <section class="min-h-screen flex flex-col relative overflow-hidden">
    ...
    <div class="absolute inset-0 overflow-hidden">
      ...
      <!-- Floating orbs -->
      <div
        class="absolute top-1/4 left-1/4 w-[500px] h-[500px] rounded-full bg-primary/5 blur-[120px] animate-pulse"
      />
      <div
        class="absolute bottom-1/4 right-1/4 w-[400px] h-[400px] rounded-full bg-primary/5 blur-[100px] animate-pulse"
        style="animation-delay: 1s"
      />
      <div
        class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] rounded-full bg-primary/5 blur-[150px] animate-pulse"
      ...
      <div class="max-w-[1200px] mx-auto px-6 w-full">
        ...
        >
          ...
          <div class="flex justify-center mb-8">
            <div
              class="inline-flex items-center gap-2 px-4 py-2 rounded-full border border-border bg-card/50 backdrop-blur-sm"
            >
          ...
          <h1 class="text-center mb-6">
            <span
              class="block text-5xl md:text-7xl lg:text-8xl font-bold text-foreground tracking-tight mb-4"
            >
              Enzo DI GIOVANNI
            </span>
          ...
          >
            <div
              ...
              :ref="
                (el) => {
                  statRefs[i] = el as HTMLElement | null;
                }
              "
              :data-stat-index="i"
              class="text-center"
    ...
    <!-- Side decorations -->
    <div
      class="hidden lg:block absolute left-8 top-1/2 -translate-y-1/2 text-muted-foreground/20 text-xs font-mono writing-vertical"
    >
      PORTFOLIO 2026
    </div>
    <div
      class="hidden lg:block absolute right-8 top-1/2 -translate-y-1/2 text-muted-foreground/20 text-xs font-mono writing-vertical"
    >
      FULL-STACK DEV
    </div>
  </section>
