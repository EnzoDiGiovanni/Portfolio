...
onMounted(() => {
  if (featuredRef.value) {
    const obs = new IntersectionObserver(
      ([entry]) => {
        if (entry.isIntersecting) featuredVisible.value = true;
      },
      { threshold: 0.2 },
    );
...
<template>
  <section id="projects" class="py-24 md:py-32 relative">
    ...
    <div class="absolute inset-0 overflow-hidden">
      <div
        class="absolute top-1/2 left-0 w-1/3 h-1/2 bg-primary/5 rounded-full blur-[200px]"
      />
    </div>
    ...
    <div class="max-w-[1200px] mx-auto px-6 relative">
      <!-- Header -->
      <div
        class="flex flex-col md:flex-row md:items-end md:justify-between mb-16"
      >
        <div>
          <span
        ...
        <p class="text-muted-foreground max-w-md mt-4 md:mt-0 md:text-right">
          Des applications conçues pour résoudre de vrais problèmes avec
      ...
      <div v-if="featuredProject" class="mb-8">
        ...
        >
          <div class="grid lg:grid-cols-2 gap-0">
            <!-- Image side -->
            <div
              class="relative h-64 lg:h-auto overflow-hidden bg-gradient-to-br from-card to-background"
            >
              <div
              ...
              >
                ...
                <div class="w-full h-full flex items-center justify-center">
                  <div class="relative">
                    <div
                      class="w-48 h-32 rounded-lg bg-background border border-border shadow-2xl transform -rotate-6"
                    />
                    <div
                      class="absolute top-4 left-4 w-48 h-32 rounded-lg bg-card border border-border shadow-2xl transform rotate-3"
                    />
                    <div
                      class="absolute top-8 left-8 w-48 h-32 rounded-lg bg-primary/10 border border-primary/30 shadow-2xl"
                    />
                  </div>
                </div>
              </div>
              ...
              <div class="absolute top-6 left-6">
                <span
                  class="px-4 py-2 rounded-full bg-primary text-primary-foreground text-xs font-semibold uppercase tracking-wider"
                >
                  Projet Vedette
                </span>
              </div>
            ...
            <div class="p-8 lg:p-12 flex flex-col justify-center">
              ...
              <h3
                class="text-3xl lg:text-4xl font-bold text-foreground mb-4 group-hover:text-primary transition-colors"
              >
              ...
              <div class="flex gap-4">
                <AppButton href="#" variant="primary" class="group/btn">
                  Voir le projet
                  <ArrowUpRight
                    class="ml-2 h-4 w-4 group-hover/btn:translate-x-0.5 group-hover/btn:-translate-y-0.5 transition-transform"
                  />
                </AppButton>
      ...
      <div class="grid md:grid-cols-3 gap-6">
        ...
        >
          <!-- Image placeholder -->
          <div
            class="relative h-48 overflow-hidden bg-gradient-to-br from-card to-background"
          >
            <div
              class="absolute inset-0 bg-gradient-to-t from-card via-transparent to-transparent z-10"
            />
            <div
              class="absolute inset-0 transition-transform duration-500"
              :style="{
                transform: cardHovered[index] ? 'scale(1.1)' : 'scale(1)',
              }"
            >
              <div class="w-full h-full flex items-center justify-center">
                <div
                  class="w-32 h-20 rounded-lg bg-primary/10 border border-primary/20 shadow-lg"
                />
              </div>
            </div>
          ...
          <div class="p-6">
            <span
              ...
              class="text-xs text-primary font-medium mb-2 block"
            >
              {{ project.metrics }}
            </span>
            <h3
      ...
      <div class="mt-12 text-center">
        ...
        >
          ...
          <ArrowUpRight
            class="ml-2 h-4 w-4 group-hover:translate-x-0.5 group-hover:-translate-y-0.5 transition-transform"
          />
        </AppButton>
      </div>
    </div>
